...
// runtime & cadence
//const HANDS_STEP_MS  = 100;   // run ~6 fps (tablet-safe)
const HANDS_FAST_MS = 66;    // ~15 fps when hands are present
const HANDS_IDLE_MS = 180;   // ~5–6 fps when idle
const HANDS_CACHE_MS = 800;
const HANDS_SEND_MS  = 600;

// model + input
const HANDS_MODEL_URL = "/mp/hand_landmarker.task"; // <-- host this file
const HANDS_MAX_NUM   = 1;     // track only 1 hand for perf
const HANDS_IMAGE_SIDE = 256;  // downscale side; ok at 256–320
...
function classifyWave(landmarks, now){
  try {
    ...
    if (flips >= 1) return {ok:true, type:"wave", score: Math.min(1, 0.3 + flips*0.2)};
    return {ok:false};
...
function classifyThumbsUp(landmarks){
  try {
    ...
    const wrist = landmarks[MP.WRIST];
    if (!(thumbTip && indexMcp && indexTip && pinkyMcp && wrist)) return {ok:false};

    const vIdx = v2(indexMcp.x - wrist.x, indexMcp.y - wrist.y);
    const vPky = v2(pinkyMcp.x - wrist.x, pinkyMcp.y - wrist.y);
    const span = Math.hypot(vIdx.x - vPky.x, vIdx.y - vPky.y);
    ...
    // Loosen a bit so it locks quicker on static poses:
    const thumbAbove = thumbTip.y < indexMcp.y - 0.008;   // a bit looser
    const open = dist(thumbTip, indexTip) > 0.032;        // allow smaller gap
    const bigEnough = span > 0.035;                       // allow smaller hand
    const orientedUp = axisCosToVertical > 0.82;          // ~< 34° from vertical

    // Fast-path: palm upward-ish also helps (index MCP below wrist a bit)
    const palmUpish = (indexMcp.y > wrist.y + 0.01);

    if (thumbAbove && open && bigEnough && orientedUp) {
      const openness = Math.max(0, Math.min(1, (dist(thumbTip, indexTip) - 0.032) / 0.12));
      const orientBoost = Math.min(0.25, Math.max(0, (axisCosToVertical - 0.78) * 1.2));
      const s = Math.max(0, Math.min(1, openness + orientBoost));
...
const WAVE_BOOT_MS = 400;        // after startup/change, allow wave to “claim” quickly
const WAVE_GRACE_MS = 900;       // how long wave is allowed to win ties/near-ties
...
const GESTURE_PRIORITY   = ["wave", "thumbs_up", "paper", "rock", "scissors"];
...
function pickStableGesture(now, win, prevStable) {
  ...
  if (waveStats) {
    ...
    if (waveRecent && prevIsWavey) {
      ...
      const nearTie =
        (best.type !== "wave") &&
        (waveCand.count >= best.count - 1); // allow off-by-one
      if (best.type !== "wave" && nearTie) {
        best = waveCand;
      }
    }
  }
  ...
  const strong = (byType.get(best.type)?.best ?? 0) >= 0.68 && best.count >= 2;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on("audio_chunk_received", (pkt) => {
      try {
        ...
        console.warn("[audio_chunk_received] decode error:", e);
      }
    });
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      const now = performance.now();
      if (now - lastRun < LOOP_STEP_MS || !video.videoWidth || video.readyState < 2 || detecting) return;
      frameCount++;
      ...
      try {
        ...
        for (let k = 0; k < tracked.length; k++) {
          ...
          const gestureIcon =
            freshGesture && zone === "green"
              ? (freshGesture.type === "wave" ? "👋"
                  : freshGesture.type === "thumbs_up" ? "👍"
                  : "🤟")
              : "";
        ...
        trackedFacesRef.current = peopleForPost.map(p => ({
          cx: p._cx, cy: p._cy, yawDeg: p.yawDeg, pitchDeg: p.pitchDeg
        }));
        ...
        // Adaptive cadence: fast right after landmarks seen; slow otherwise
        const handsDesiredStep =
          (now - (lastLmSeenTsRef.current || 0) <= 800) ? HANDS_FAST_MS : HANDS_IDLE_MS;
          ...
          try {
            ...
            if (lm && lm.length >= 21) {
              ...
              try {
                ...
                console.warn("[gesture] stabilizer error:", e);
              }

              if (stable) {
                const prev = stableGestureRef.current;
                const changed = !prev || prev.type !== stable.type;
                stableGestureRef.current = stable;
                lastGestureRef.current = { ...stable };

                if (changed &&
                    (now - (lastGestureSentRef.current || 0)) >= HANDS_SEND_MS &&
                    !speakingRef.current) {
                  try {
                    const focusTarget =
                      (Number.isFinite(focusIndex) && focusIndex >= 0)
                        ? (peopleForPost[focusIndex]?.name || peopleForPost[focusIndex]?.gid || null)
                        : null;
                    socketRef.current?.emit?.("gesture_event", {
                      sessionId: sessionId || "web-" + deviceId,
                      deviceId,
                      gesture: { type: stable.type, score: stable.score },
                      at: Date.now(),
                      focusIndex,
                      focusTarget,
                    });
            ...
            // keep faces running even if hands throws
            // console.warn("[hands] detect error:", e);
          }
        } else {
          // when not eligible, decay history
          if (lastGestureRef.current && (now - lastGestureRef.current.t) > HANDS_CACHE_MS) {
            lastGestureRef.current = null;
          }
        }

        // === Attention gate: greet only if centered & steady ===
        let invite = null;
        ...
        try {
          ...
          for (const p of peopleForPost) {
            const key = (p.name || p.gid) ?? null;
            if (!key) continue;
            seenKeys.add(key);

            // must be in green and facing fairly straight
            const facing =
              Math.abs(p.yawDeg ?? 999) <= FACING_YAW_MAX_DEG &&
              Math.abs(p.pitchDeg ?? 999) <= FACING_PITCH_MAX_DEG &&
              p.zone === "green";

            const rec = attentionMapRef.current.get(key) || {
              count: 0, lastInviteTs: 0, invites: 0, seenAt: now
            };
            rec.seenAt = now;

            // (optional) reset if they were gone for a while
            if (now - (rec.lastSeenAt || 0) > NOT_SEEN_RESET_MS) {
              rec.invites = 0;
              rec.count = 0;
            }
            rec.lastSeenAt = now;
            ...
            if (facing) {
              ...
              const canInvite =
                !invite &&                                   // only pick one per frame
                rec.count >= ATTEND_MIN_FRAMES &&
                (now - (rec.lastInviteTs || 0)) > GREET_COOLDOWN_MS &&
                (rec.invites || 0) < MAX_INVITES_PER_PERSON;

              if (canInvite) {
                invite = { gid: p.gid || null, name: p.name || null };
                rec.lastInviteTs = now;
                rec.invites = (rec.invites || 0) + 1;
              }
            } else {
              rec.count = 0; // they looked away → reset streak
            }

            attentionMapRef.current.set(key, rec);
          }

          // prune identities not seen this frame
          for (const k of attentionMapRef.current.keys()) {
            if (!seenKeys.has(k)) attentionMapRef.current.delete(k);
          }
        } catch {}

        // Small HUD tag (bottom-left) — reuse the existing gesturePayload
        if (gesturePayload) {
          const msg = `gesture: ${gesturePayload.type}`;
          ctx.save();
          ctx.font = "bold 16px system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif";
          const w = ctx.measureText(msg).width + 14;
          ctx.fillStyle = "rgba(34,197,94,0.8)";
          ctx.fillRect(10, canvas.height - 34, w, 26);
          ctx.fillStyle = "#fff";
          ctx.fillText(msg, 17, canvas.height - 26);
          ctx.restore();
        }

        if (import.meta.env.DEV && performance.now() % 1000 < 16) {
          // roughly once per second
          console.log("focusIndex:", focusIndex, "gestureTarget:",
            Number.isFinite(focusIndex) && focusIndex >= 0
              ? (peopleForPost[focusIndex]?.name || peopleForPost[focusIndex]?.gid || null)
              : null
          );
        ...
        // --- Session rotation based on group change (won't break drawing) ---
        try {
          const sig = groupSignature(peopleForPost);
          maybeRotateSession(sig, peopleForPost);
        } catch (e) {
          console.warn("[session-rotate] skipped:", e);
        }

        // (unchanged) now emit to server — but with richer payload
        emitCrowdThrottled({
        sessionId: sessionId || "web-" + deviceId,
        deviceId,

        // status
        allGreen: total > 0 && green === total,
        totals: { all: total, green, red },

        // per-face telemetry
        faces: peopleForPost.map((p) => ({
          name: p.name || null,
          gid: p.gid || null,
          distanceM: p?.posCam?.z ?? null,
          ...
          yawDeg: p.yawDeg,
          pitchDeg: p.pitchDeg,
          posCam: p.posCam,            // {x,y,z}
          mouthActivity: p.mouthActivity,
          centerNorm: p.centerNorm,
        })),
        ...
        // gestures (stable or null)
        gesture: gesturePayload,

        // environment snapshot (state/UI)
        environment: {
          timeISO: new Date().toISOString(),
          localTime: clock.toLocaleString(),
          locationLabel,
          weatherLabel,
          model: modelQuick,
          ttsProvider: ttsProviderQuick,
          voice: geminiVoiceQuick,
          languageCode: languageCodeQuick,
          captions,
          systemInstruction,
          // camera alignment + ranges
          fovHdeg, fovVdeg, panOffsetDeg, tiltOffsetDeg,
          greenMaxM: greenMaxMRef.current,
          redCutoffM,
          // runtime info
          backend, // tfjs backend
        },
        ...
        // Try one-time WebGL -> WASM fallback, but do it *serially*
        if (!triedFallbackRef.current && tf.getBackend?.() === "webgl") {
          triedFallbackRef.current = true;

          backendSwitchingRef.current = true;
          backendReadyRef.current = (async () => {
            try {
              await tf.setBackend("wasm");
              await tf.ready();
              backendNameRef.current = tf.getBackend();
              setBackend(backendNameRef.current);
              console.log("[tfjs] switched backend ->", backendNameRef.current);
            } finally {
              backendSwitchingRef.current = false;
            }
          })();

          // Bail this frame so we don't detect during the switch
          detecting = false;
          return;
        }
      } finally {
    ...
    <main className="app">
      <div className="page2">
        ...
        <div className="leftcol">
          ...
          {/* Row 2: DISTANCE CONTROLS — two neat one-line rows */}
          <div className="panel">
